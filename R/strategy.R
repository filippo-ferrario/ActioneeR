...
strategy_side_line<-function(fov_L, dist, cam_spacing, min_camera_overlap=NULL, n_cams, corridor_length=NULL){
	...
	rig_swath<-swath(fov_L=fov_L, n_cams=n_cams, dist=dist, cam_spacing=cam_spacing)
	hlf_sw<- rig_swath/2
	...
	if (is.na(cam2rig_overlap) | (cam2rig_overlap < cam_ov) ) {
		...
		dists_to_trans<-Xswcnt+(1:extra_pass)
	...
	res2[7,]<-c('distance of extra passages from transect',dists_to_trans)
